...
export default function JournalScreen({ user, onBack }) {
  ...
  async function handleSaveManual(content) {
    ...
    const { data, error } = await supabase
      .from('journal_entries')
      .insert({ user_id: user.id, type: 'manual', content })
      .select()
      .single()
  ...
  const atLimit   = entries.length >= FREE_LIMIT
  const fillPct   = Math.min((entries.length / FREE_LIMIT) * 100, 100)

  return (
    <div style={{
      minHeight: '100vh', paddingTop: '96px', paddingBottom: '120px',
      ...
      <div style={{ maxWidth: '580px', margin: '0 auto', padding: '0 32px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
            <div>
              ...
              <p style={{
                ...
                {loading ? '…' : `${entries.length} / ${FREE_LIMIT} entries`}
              </p>
            </div>
            ...
            {!loading && entries.length > 0 && (
              ...
              >
                <div style={{
                  ...
                  background: 'var(--flz-border)', borderRadius: '2px', overflow: 'hidden',
                }}>
                  <div style={{
                    height: '100%',
                    width: `${fillPct}%`,
                    background: atLimit ? '#ef4444' : 'var(--flz-text)',
                    borderRadius: '2px',
                ...
                {atLimit && (
                  <span style={{
                    fontFamily: 'Inter, system-ui, sans-serif',
                    fontSize: '0.62rem', color: '#ef4444', letterSpacing: '0.04em',
                  }}>
                    Limit reached
                  </span>
                )}
        ...
        {loading ? (
          ...
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.6, ease: EASE_CALM, delay: 0.2 }}
            style={{ borderTop: '1px solid var(--flz-border-soft)', paddingTop: '28px' }}
          >
            <p style={{
              fontFamily: 'Inter, system-ui, sans-serif',
              fontSize: '0.9rem', color: 'var(--flz-text-muted)', lineHeight: 1.8, margin: 0,
            }}>
        ...
        {!atLimit && !composing && (
          <motion.button
            ...
            style={{
              ...
              background: 'var(--flz-text)', border: 'none',
              color: 'var(--flz-bg)', fontSize: '1.5rem', lineHeight: 1,
